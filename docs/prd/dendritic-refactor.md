...
modules/
  features/
    ...
    wireguard-p1g3.nix                 # P1 only: WireGuard interface config for p1g3
    wireguard-t14s.nix                 # T14s only: WireGuard interface config for t14s
    nfs.nix                            # Shared: NFS mount for /mnt/nas (both hosts)
    luks-p1g3.nix                      # P1 only: LUKS device UUID for secondary disk
...
### Host-Specific Module Selection

- **WireGuard:** Separate files per host (`wireguard-p1g3.nix`, `wireguard-t14s.nix`). Config paths differ per host, and a host might not have WireGuard at all. Simple split beats parametric module for a single path.
- **LUKS:** Separate files per host (`luks-p1g3.nix`). UUIDs are inherently host-specific.
...
**Rationale:** The community standard for multi-host flake configs is explicit per-host module selection. No one uses automatic filtering at the import-tree level — hosts explicitly list what they import. This makes it clear at a glance which modules a host uses and avoids silent config bloat.
...
- Extract from `base.nix`: `nvidia.nix`, `networking.nix`, `audio.nix`, `bluetooth.nix`, `docker.nix`, `services.nix`, `security.nix`, `power.nix`, `packages-system.nix`, `nfs.nix`, `graphics.nix`, `hyprland-system.nix`
- Update host assembly in `flake.nix`
- Verify: build succeeds

**Phase 2.5: ✅ Harden for multi-host**
- Move LUKS UUIDs from `base.nix` to `luks-p1g3.nix` (P1-only)
- Split WireGuard from `networking.nix` into `wireguard-p1g3.nix` (P1-only)
- Move `networking.hostName` from `base.nix` to host assembly in `flake.nix`
- Remove dead `modules/hosts/p1g3/default.nix` (assembly lives in `flake.nix`)
- Simplify `loadFeature` to one-liner (drop dead `isFunction` branch)
